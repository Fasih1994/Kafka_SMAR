...
CONSUMER_GROUP="twitter_get_post_data"
...
if [ $UNIQUE_CONSUMER_COUNT -lt 1 ]; then
    ...
    $ROOT_DIR/venv/bin/python -u $ROOT_DIR/scripts/twitter/get_post_data.py > "$ROOT_DIR/scripts/logs/$current_time-$CONSUMER_GROUP.log"
else
