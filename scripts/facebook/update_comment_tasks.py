...
from scripts.config import (
    ...
    FACEBOOK_POSTS_TOPIC,
    FACEBOOK_COMMENT_TASKS_TOPIC
)

from scripts.utils import (
    facebook_post_deserializer,
    task_serializer,
    string_serializer
)
...
def update_comment_task(msg=None, post_data: dict=None):
    update_url = f"https://api.data365.co/v1.1/facebook/post/{post_data['id']}/update"
    params = {
    ...
    if r.status_code == 202:
        ...
        producer.produce(
            ...
            value=task_serializer(data, SerializationContext(FACEBOOK_COMMENT_TASKS_TOPIC, MessageField.VALUE)),
...
def main():
    consumer_conf['group.id'] = 'facebook_update_comment_tasks'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([FACEBOOK_POSTS_TOPIC])
    WAIT_COUNT = 0
    ...
    while True:
        try:
            ...
            if msg is not None:
                post_data = facebook_post_deserializer(msg.value(), SerializationContext(msg.topic(), MessageField.VALUE))
