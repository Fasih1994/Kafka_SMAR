...
from scripts.config import (
    consumer_conf,
    producer_conf,
    FACEBOOK_POST_TASKS_TOPIC,
    FACEBOOK_POST_TASKS_FINISHED_TOPIC
)
        ...
        if response['data']['status']=='finished':
           # if task is finished send to finished Topic
            ...
            producer.produce(
                FACEBOOK_POST_TASKS_FINISHED_TOPIC,
                key=msg.key(),
                value=task_serializer(task_data, SerializationContext(FACEBOOK_POST_TASKS_FINISHED_TOPIC, MessageField.VALUE)),
            ...
            producer.produce(
                FACEBOOK_POST_TASKS_TOPIC,
                key=msg.key(),
                value=task_serializer(task_data, SerializationContext(FACEBOOK_POST_TASKS_TOPIC, MessageField.VALUE)),
                on_delivery=delivery_report
            )
            producer.flush()
...
def main():
    consumer_conf['group.id'] = 'facebook_check_post_tasks'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([FACEBOOK_POST_TASKS_TOPIC])
    WAIT_COUNT = 0
        ...
        try:
            ...
            if msg is not None:
                ...
                if task_data['platform'] == 'facebook':
                    ...
                    print(f'Platform is {task_data["platform"]}')
                    consumer.commit()
    ...
    consumer.close()
    logger.info("All task completed for facebook posts.")
