...
def update_post_task(msg=None, term_data: dict=None):
    search_type='top'
    update_url = "https://api.data365.co/v1.1/facebook/search/{search_request}/posts/{search_type}/update"
    update_url = update_url.format(
        search_type=search_type,
        search_request=term_data['key_term']
    )
    ...
    if r.status_code == 202:
        ...
        producer.produce(
            FACEBOOK_POST_TASKS_TOPIC,
            key=msg.key(),
            value=task_serializer(data, SerializationContext(FACEBOOK_POST_TASKS_TOPIC, MessageField.VALUE)),
...
def main():
    consumer_conf['group.id'] = 'facebook_update_post_tasks0'
    ...
    while True:
        try:
            ...
            if msg is not None:
                ...
                WAIT_COUNT=0
                if term_data['platform'] == 'facebook':
                    try:
