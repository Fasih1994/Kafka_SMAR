...
from scripts.config import (
    ...
    consumer_conf,
    producer_conf,
    LINKEDIN_COMMENT_TASKS_FINISHED_TOPIC,
    LINKEDIN_COMMENTS_TOPIC
)

from scripts.utils import (
    task_deserializer,
    linkedin_comment_serializer
)
...
def produce_comment(msg=None, comments: list=None, url: str=None):
    ...
    for comment in comments:
        producer.produce(
            LINKEDIN_COMMENTS_TOPIC,
            key=msg.key(),
            value=linkedin_comment_serializer(
                comment,
                SerializationContext(LINKEDIN_COMMENTS_TOPIC, MessageField.VALUE)),
...
def main():
    consumer_conf['group.id'] = 'linkedin_get_comments_data'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([LINKEDIN_COMMENT_TASKS_FINISHED_TOPIC])
    WAIT_COUNT = 0
    ...
    while True:
        try:
            ...
            if msg is not None:
                ...
                WAIT_COUNT=0
                if task_data['platform'] == 'linkedin':
                    try:
