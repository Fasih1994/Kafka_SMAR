           ...
           # if task is finished send to finished Topic
            ...
            producer.produce(
                ...
                value=task_serializer(
                    task_data,
                    SerializationContext(LINKEDIN_POST_TASKS_FINISHED_TOPIC, MessageField.VALUE)),
            ...
            producer.produce(
                LINKEDIN_POST_TASKS_TOPIC,
                key=msg.key(),
                value=task_serializer(task_data, SerializationContext(LINKEDIN_POST_TASKS_TOPIC, MessageField.VALUE)),
                on_delivery=delivery_report
            )
            producer.flush()
...
def main():
    consumer_conf['group.id'] = 'linkedin_check_post_tasks'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([LINKEDIN_POST_TASKS_TOPIC])
    WAIT_COUNT = 0
