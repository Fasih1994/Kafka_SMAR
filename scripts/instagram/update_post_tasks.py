...
def update_post_task(msg=None, term_data: dict=None):
    ...
    if r.status_code == 202:
        ...
        producer.produce(
            INSTAGRAM_POST_TASKS_TOPIC,
            key=msg.key(),
            value=task_serializer(data, SerializationContext(INSTAGRAM_POST_TASKS_TOPIC, MessageField.VALUE)),
...
def main():
    consumer_conf['group.id'] = 'instagram_update_post_tasks'
    ...
    while True:
        try:
            ...
            if msg is not None:
                ...
                WAIT_COUNT=0
                if term_data['platform'] == 'instagram':
                    try:
