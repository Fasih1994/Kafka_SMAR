...
def update_comment_task(msg=None, post_data: dict=None):
    update_url = f"https://api.data365.co/v1.1/instagram/post/{post_data['id']}/update"
    params = {
...
def main():
    consumer_conf['group.id'] = 'instagram_update_comment_tasks'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([INSTAGRAM_POSTS_TOPIC])
    WAIT_COUNT = 0
    ...
    while True:
        try:
            ...
            if msg is not None:
                post_data = instagram_post_deserializer(
                    msg.value(),
