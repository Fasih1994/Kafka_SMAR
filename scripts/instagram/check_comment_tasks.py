           ...
           # if task is finished send to finished Topic
            ...
            producer.produce(
                ...
                value=task_serializer(
                    task_data,
                    SerializationContext(INSTAGRAM_COMMENT_TASKS_FINISHED_TOPIC, MessageField.VALUE)),
            ...
            producer.produce(
                ...
                value=task_serializer(
                    task_data,
                    SerializationContext(INSTAGRAM_COMMENT_TASKS_TOPIC, MessageField.VALUE)),
                on_delivery=delivery_report
            )
            producer.flush()
...
def main():
    consumer_conf['group.id'] = 'instagram_check_comment_tasks'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([INSTAGRAM_COMMENT_TASKS_TOPIC])
    WAIT_COUNT = 0
    ...
    consumer.close()
    logger.info("All task completed for instagram comments.")
