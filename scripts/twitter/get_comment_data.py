...
def produce_comment(msg=None, comments: list=None, url: str=None):
    ...
    for comment in comments:
        producer.produce(
            COMMENTS_TOPIC,
            key=msg.key(),
            value=comment_serializer(comment, SerializationContext(COMMENTS_TOPIC, MessageField.VALUE)),
...
def main():
    consumer_conf['group.id'] = 'twitter_get_comments_data'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([COMMENT_TASKS_FINISHED_TOPIC])
    WAIT_COUNT = 0
