...
from scripts.config import (
    consumer_conf,
    producer_conf,
    TIKTOK_POST_TASKS_TOPIC,
    TIKTOK_POST_TASKS_FINISHED_TOPIC
)
           ...
           # if task is finished send to finished Topic
            producer.produce(
                ...
                value=task_serializer(
                    task_data,
                    SerializationContext(TIKTOK_POST_TASKS_FINISHED_TOPIC, MessageField.VALUE)),
            ...
            producer.produce(
                ...
                value=task_serializer(
                    task_data,
                    SerializationContext(TIKTOK_POST_TASKS_TOPIC, MessageField.VALUE)),
                on_delivery=delivery_report
            )
            producer.flush()
...
def main():
    consumer_conf['group.id'] = 'tiktok_check_post_tasks'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([TIKTOK_POST_TASKS_TOPIC])
    WAIT_COUNT = 0
    ...
    consumer.close()
    logger.info("All task completed for tiktok posts.")
