...
from scripts.config import (
    ...
    TIKTOK_POSTS_TOPIC,
    TIKTOK_COMMENT_TASKS_TOPIC
)

from scripts.utils import (
    tiktok_post_deserializer,
    task_serializer,
    string_serializer
)
...
def update_comment_task(msg=None, post_data: dict=None):
    update_url = "https://api.data365.co/v1.1/tiktok/profile/{profile_id}/feed/posts/{post_id}/update"
    params = {
    ...
    if r.status_code == 202:
        ...
        producer.produce(
            ...
            value=task_serializer(
                data,
                SerializationContext(TIKTOK_COMMENT_TASKS_TOPIC, MessageField.VALUE)),
...
def main():
    consumer_conf['group.id'] = 'tiktok_update_comment_tasks'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([TIKTOK_POSTS_TOPIC])
    WAIT_COUNT = 0
    ...
    while True:
        try:
            ...
            if msg is not None:
                post_data = tiktok_post_deserializer(
                    msg.value(),
