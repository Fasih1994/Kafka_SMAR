...
from scripts.config import (
    ...
    TIKTOK_COMMENTS_TOPIC
)

from scripts.utils import (
    task_deserializer,
    tiktok_comment_serializer
)
...
def produce_comment(msg=None, comments: list=None, url: str=None):
    ...
    for comment in comments:
        producer.produce(
            TIKTOK_COMMENTS_TOPIC,
            key=msg.key(),
            value=tiktok_comment_serializer(
                comment,
                SerializationContext(TIKTOK_COMMENTS_TOPIC, MessageField.VALUE)),
...
def main():
    consumer_conf['group.id'] = 'tiktok_get_comments_data'
    consumer = Consumer(consumer_conf)
    consumer.subscribe([TIKTOK_COMMENT_TASKS_FINISHED_TOPIC])
    WAIT_COUNT = 0
